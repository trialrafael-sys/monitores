...
def clean_credor(raw):
    """Remove prefixo numérico/documental antes do nome do credor PEFIN/REFIN."""
    import re as _re
    raw = raw.strip()
    if not raw: return raw
    # Pure numeric/punct prefix + 2+ spaces + name
    m = _re.match(r'^[\d./\-]+\s{2,}(.+)', raw)
    if m: return m.group(1).strip()
    # Pure numeric 8+ digit + single space + name
    m = _re.match(r'^[\d./\-]{8,}\s(.+)', raw)
    if m: return m.group(1).strip()
    # Short 4-7 digit prefix + space + capital name
    m = _re.match(r'^(\d{4,7})\s([A-Z].+)', raw)
    if m: return m.group(2).strip()
    # Alphanum code with digits + 2+ spaces + name  (e.g. '7TA31851001     UNIDAS')
    m = _re.match(r'^([A-Z0-9./\-]+)\s{2,}([A-Z].+)', raw)
    if m and _re.search(r'\d', m.group(1)): return m.group(2).strip()
    # Alphanum code with digits + single space + name
